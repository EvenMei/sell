	...
	t.extend(i.prototype, {
		init: function() {
			...
			a.settings.init && a.settings.init(), a.cart_empty(), a.product.each(function(e) {
				var n = t(this),
					...
					mei=n.find("input[name='product_id']" ).val(), // data-id
					c = i.text(),
				...
				s.on("click touchend", function(t) {
					...
					d.val();
					return 0 !== l.length ? p !== u ? a.add_to_cart(c, o, u,mei) : _.find("input").val(function(t, e) {
						return parseInt(e, 10) + 1
					}) : (a.add_to_cart(c, o, u,mei), "false" === a.settings.permanent_cart_buttons && a.add_cart_buttons(), "true" === a.settings.display_total_value && "false" === a.settings.permanent_total_value && a.append_total()), l = a.cart.find("li"), d = l.find("." + a.settings.cart + "-input"), s = l.length, a.quantity_change(d), a.calculate_total_value(), a.remove_item(s), a.settings.after_add_to_cart && a.settings.after_add_to_cart(), !1
				})
			})
		},


		add_to_cart: function(t, e, a,pid) {  //加一个参数用作data-id
			var n = this;
			n.cart.append("true" === n.settings.currency_after_number ? '<li    productid="'+pid+'"  class="animated ' + n.settings.animation + '" data-id=' + a +     " ><span class=" + n.settings.cart + "-name>" + t + "</span><span class=" + n.settings.cart + "-price>" + e + n.settings.currency + '</span><input type="number" min="1" value="1" class=' + n.settings.cart + "-input><button class=" + n.settings.cart + "-remove>&times;移除</button></li>" : '<li  productid="'+pid+'" class="animated ' + n.settings.animation + '" data-id=' + a +  "><span class=" + n.settings.cart + "-name>" + t + "</span><span class=" + n.settings.cart + "-price>" + n.settings.currency + e + '</span><input type="number" min="1" value="1" class=' + n.settings.cart + "-input><button class=" + n.settings.cart + "-remove>移除</button></li>"), n.storage_set()
		},
		...
		checkout: function() {
			var e, a = this,
				...
				openid= document.getElementById("input_openid").value;
			i = t("<form />"),
				r = {
					name: "orderDetailList",    // 原来是 productList
					quantity: "orderDetailList",  // 原来是 productList
					amount: "amount"
				};
			t.extend(n, a.settings.paypal), i.attr("action", "http://xiaomei.natapp1.cc/sell/buyer/submit"), i.attr("method", "post");
			i.append('<input type="hidden" name="openid" value= '+openid+'>');
//			for(var s in n) e = t('<input type="hidden" />'), e.attr("name", s), e.attr("value", n[s]), i.append(e);
			a.cart.find("li").each(function(e) {
				var n = t(this),
					pid=n.attr("productid"),
...
//						}),
						quantity: t("<input />", {
							type: "hidden",
							name: r.quantity + "[" + e +"]"+".productQuantity",
							value: o
						})
					};
				i.append(u.name).append(u.quantity)   //.append(u.amount)
		...
		create_storage_cart: function() {
			for(var t = this, e = t.storage_get(), a = 0, n = e.items.length; n > a; a++) {
				var i = e.items[a].id,
					...
					c = e.items[a].input,
					l = e.items[a].pid;
//					pid=e.items[a].pid;  //获取productId
				t.cart.append('<li  productid="'+l+'" class="animated ' + t.settings.animation + '" data-id=' + i + "><span class=" + t.settings.cart + "-name>" + r + "</span><span class=" + t.settings.cart + "-price>" + s + '</span><input type="number" min="1" value="' + c + '" class=' + t.settings.cart + "-input><button class=" + t.settings.cart + "-remove>移除</button></li>")
			}
		},
